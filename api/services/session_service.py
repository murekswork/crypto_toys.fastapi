...
class SessionService:
    ...
    async def get_follow_list(self, list_name) -> FollowListSchema:
        if self.follow_lists != '{}':
            ...
            return FollowListSchema(name=list_name,
                                    coins=request_follow_list)

        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail='You dont have any follow lists!')

    async def delete_follow_list(self, list_name: str) -> list[
            FollowListSchema]:
        if self.follow_lists != '{}':
    ...
    async def add_to_follow_list(self, list_name: str, coin_id: int) -> FollowListSchema:
